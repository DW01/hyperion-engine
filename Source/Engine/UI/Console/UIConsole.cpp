...
#include <UIConsole.generated.inl>
...
public:
    ...
    void SyncUpdates()
    {
        ...
        if (m_numQueuedEntries.Get(MemoryOrder::RELAXED) == 0)
        {
            return;
        }
        ...
        {
            m_dataSource->Push(entry.uuid, BoxedValue(entry), UUID::Invalid());
        }
      ...
      m_textbox(nullptr)
{
    ...
    m_loggerRedirectId = Logger::GetInstance().GetOutputStream()->AddRedirect(
        ...
        {
            UIConsole* consoleUi = (UIConsole*)context;
...
void UIConsole::Init()
{
    ...
    Handle<UIDataSource> dataSource = MakeHandle<UIDataSource>(
        UIElementFactoryRegistry::GetInstance().GetFactories<ConsoleHistoryEntry>(),
        [this](UIObject* parent, const BoxedValue& value, const BoxedValue& context) -> Handle<UIObject>
        {
        ...
        [](UIObject* uiObject, const BoxedValue& value, const BoxedValue& context) -> void
        {
        });
...
void UIConsole::Update_Internal(float delta)
{
...
bool UIConsole::NeedsUpdate() const
{
...
MaterialParameters UIConsole::GetMaterialParameters() const
{
    return UIObject::GetMaterialParameters();
}

#pragma endregion UIConsole
